...
# --- WO File Analysis: Site_ID & STD_RFO Breakdown ---

# 1. Total distinct Site_ID count and list
...
ax1.set_xlabel('Site_ID', fontsize=12)
...
ax2.set_xlabel('Site_ID', fontsize=12)
...
ax3.set_xlabel('Site_ID', fontsize=12)
...
<ul style="font-size:0.95rem;">
    <li><b>Total Distinct Site_IDs:</b> {total_distinct_sites}</li>
</ul>
<h4>Aggregated WO Stats by Site_ID and STD_RFO</h4>
<div style="max-height:350px;overflow:auto;">
{wo_grouped_table_html}
</div>
<h4>Charts (Click to Expand)</h4>
{''.join(popup_htmls)}
<p style="font-size:0.9rem;"><em>Interpretation:</em> These charts and tables help identify sites or Root_Causes with frequent or long-duration outages, and the relationship between repair time and failure frequency. Click on each chart for a larger view.</p>
<hr>
"""
...
<p style="font-size:0.9rem;"><em>Interpretation:</em> MTTR reflects the average time to resolve incidents, while MTBF indicates the average interval between failures. High MTTR or low MTBF in specific Root_Causes or sites highlights areas for process improvement and preventive maintenance focus.</p>
<hr>
"""
...
# Add 'Date' column from 'Raise_Time' to df_filtered for temporal analysis, coercing errors to NaT.
df_filtered['Raise_Time_dt'] = pd.to_datetime(df_filtered['Raise_Time'], errors='coerce')
...
group_cols = ['Render', 'State/Division', 'Sub_Office', 'Township']

# Calculate total unique sites for each group.
total_sites = df_filtered.groupby(group_cols)['Site_ID'].nunique().reset_index(name='Total Sites')
# Calculate unique sites that are currently 'down' (i.e., 'Raise_Time' is not 'Active' or is null).
down_sites = df_filtered[
    df_filtered['Raise_Time'].isnull() | (df_filtered['Raise_Time'] != 'Active')
].groupby(group_cols)['Site_ID'].nunique().reset_index(name='Current Down Site')
# Calculate unique sites that are 'online' (i.e., 'Raise_Time' is 'Active').
online_sites = df_filtered[
    df_filtered['Raise_Time'] == 'Active'
].groupby(group_cols)['Site_ID'].nunique().reset_index(name='Online Status')
...
pivot_status = summarise.pivot_table(
    index=['Render', 'State/Division', 'Sub_Office', 'Township'],
...
# 2. Bar plot: Average CA by Sub_Office and Month (months in calendar order)
...
ax2.set_title('Average CA by Sub_Office and Month', fontsize=12)
ax2.set_xlabel('Sub_Office', fontsize=10)
...
df_filtered_html = df_filtered.to_html(
    ...
    escape=False # Allows HTML content in cells, e.g., for Site_IDs in future.
)
...
<div id="detailDataModal" style="display:none; position:fixed; top:0; left:0; width:100vw; height:100vh; background:rgba(0,0,0,0.25); z-index:9999;">
  <div style="background:#fff; margin:40px auto; padding:25px; border-radius:10px; max-width:1100px; min-width:350px; position:relative; overflow-y: auto; max-height: calc(100vh - 80px); box-shadow: 0 5px 15px rgba(0,0,0,0.3);">
    ...
    <div class="column-filters-container" id="detail-column-filters">
        ...
        <label>Sub_Office: <select class="column-filter-select" data-column-name="Sub_Office"><option value="">All</option></select></label>
        <label>Township: <select class="column-filter-select" data-column-name="Township"><option value="">All</option></select></label>
        <label>Issue_Identity: <select class="column-filter-select" data-column-name="Issue_Identity"><option value="">All</option></select></label>
    </div>
...
<div id="slicerModal" style="display:none; position:fixed; top:0; left:0; width:100vw; height:100vh; background:rgba(0,0,0,0.25); z-index:9999;">
  <div style="background:#fff; margin:40px auto; padding:25px; border-radius:10px; max-width:1000px; min-width:350px; position:relative; overflow-y: auto; max-height: calc(100vh - 80px); box-shadow: 0 5px 15px rgba(0,0,0,0.3);">
    ...
    <select id="slicerSelect" style="margin:0 8px 15px 8px; font-size:0.9rem; padding: 4px; border-radius: 4px; border: 1px solid #ccc;">
      ...
      <option value="Sub_Office">Sub_Office</option>
...
arnd_total_sites = arnd_filtered_df.groupby(group_cols)['Site_ID'].nunique().reset_index(name='Total Sites')
arnd_down_sites = arnd_filtered_df[
    arnd_filtered_df['Raise_Time'].isnull() | (arnd_filtered_df['Raise_Time'] != 'Active')
].groupby(group_cols)['Site_ID'].nunique().reset_index(name='Current Down Site')
arnd_online_sites = arnd_filtered_df[
    arnd_filtered_df['Raise_Time'] == 'Active'
].groupby(group_cols)['Site_ID'].nunique().reset_index(name='Online Status')
...
for render_type in render_types:
    for state in state_filters:
        ...
        sub_offices = filtered_df_tab['Sub_Office'].unique()
        ...
        for sub_office in sub_offices:
            sub_sites = filtered_df_tab[filtered_df_tab['Sub_Office'] == sub_office]
            num_sites = sub_sites.shape[0]
            if num_sites == 0:
                continue
            
            # CRITICAL FIX: Exclude 'Active' and 'PIC_Finder is not Active' from issue counts.
            issue_counts = sub_sites[
                ~sub_sites['Issue_Identity'].isin(['Active', 'PIC_Finder is not Active'])
            ]['Issue_Identity'].value_counts()
        ...
        sns.barplot(data=sorted_df, x="Total Issues", y="Sub_Office", palette=colors, ax=ax)
        ...
        ax.set_ylabel("Sub_Office", fontsize=9)
...
# 1. Top 5 Issue Identities (EXCLUDING 'Active' and 'PIC_Finder is not Active')
top_issues_detail = df_filtered[
    ~df_filtered['Issue_Identity'].isin(['Active', 'PIC_Finder is not Active'])
]['Issue_Identity'].value_counts().head(10) # Top 10 for more detailed insight.
...
issues_by_render = df_filtered[
    ~df_filtered['Issue_Identity'].isin(['Active', 'PIC_Finder is not Active'])
].groupby('Render')['Issue_Identity'].value_counts().unstack(fill_value=0)
issues_by_render_html = issues_by_render.to_html(classes="styled-table", escape=False)

# 3. Sub_Offices with most issues (excluding 'Active' status)
# CRITICAL FIX: Ensure 'PIC_Finder is not Active' is also excluded here for consistency.
issues_per_suboffice = df_filtered[
    ~df_filtered['Issue_Identity'].isin(['Active', 'PIC_Finder is not Active'])
].groupby('Sub_Office')['Issue_Identity'].count().nlargest(5) # Top 5 now for better insight.
...
if not issues_per_suboffice.empty:
    ...
    most_problematic_suboffices_html += "<li>No specific sub-offices with issues found (excluding 'Active' and 'PIC_Finder is not Active').</li>"
most_problematic_suboffices_html += "</ul>"
...
if not issues_per_suboffice.empty:
    ...
    ax_geo_hotspots.set_ylabel('Sub_Office', fontsize=10)
...
# 4. Time-based analysis for 'Raise_Time' (temporal trends in new issues)
# Filter for actual 'down' events: where Raise_Time is a valid datetime and not 'Active' or 'PIC_Finder is not Active'.
down_events = df_filtered[
    df_filtered['Raise_Time_dt'].notna() &
    ~df_filtered['Issue_Identity'].isin(['Active', 'PIC_Finder is not Active'])
].copy()

# Count issues by month of 'Raise_Time'.
issues_by_month = down_events['Raise_Time_dt'].dt.to_period('M').value_counts().sort_index()
...
temporal_trends_chart_html = f"""
    ...
    <p style="font-size:0.9rem;">Understanding the seasonality or trend of new operational issues (based on 'Raise_Time' for non-online status events) can help in predictive maintenance and resource planning:</p>
    ...
    df_filtered['Latitude'].notnull().any() and df_filtered['Longitude'].notnull().any():
     ...
     if not geo_issues.empty:
          ...
          for _, row in geo_issues.iterrows():
                popup_text = f"""
                <b>Site_ID:</b> {row['Site_ID']}<br>
                <b>Issue:</b> {row['Issue_Identity']}<br>
                <b>Sub_Office:</b> {row['Sub_Office']}<br>
                <b>Township:</b> {row['Township']}<br>
                <b>Raise_Time:</b> {row['Raise_Time']}
                """
                folium.CircleMarker(
...
# This assumes 'Raise_Time' marks the start of an issue (if it's a timestamp).
# We are looking at sites that have a recorded 'Raise_Time' (meaning they went down)
# and their 'Issue_Identity' is not 'Active' or 'PIC_Finder is not Active'.
# Group by Issue_Identity and calculate average prolonging duration (in days)
...
prolonged_issues['Prolonging Duration'] = prolonged_issues['Raise_Time_dt'].apply(
    lambda x: now - x if pd.notna(x) else pd.Timedelta(0)
)
prolonged_issues['Prolonging Days'] = prolonged_issues['Prolonging Duration'].apply(lambda x: x.total_seconds() / (24 * 3600) if pd.notnull(x) else np.nan)
...
for render_val, render_group in prolonged_issues.groupby('Render'):
    avg_prolonging_timedelta = render_group.groupby('Issue_Identity')['Prolonging Duration'].mean().sort_values(ascending=False)
    avg_prolonging_days = render_group.groupby('Issue_Identity')['Prolonging Days'].mean().sort_values(ascending=False)
    site_counts = render_group.groupby('Issue_Identity')['Site_ID'].nunique().reindex(avg_prolonging_timedelta.index)
    ...
    for group_name, idx, bg_color, accent_color in group_labels:
        ...
        if not group_days.empty:
            ...
            ax_scatter.set_xlabel('Issue_Identity', fontsize=10)
        ...
        for issue in group_td.index:
            ...
            # Get all unique Site_IDs for this issue in this render group
            site_ids = render_group[render_group['Issue_Identity'] == issue]['Site_ID'].unique()
            site_ids_str = ", ".join(map(str, site_ids))
            # Add details/summary for Site_IDs
            site_ids_html = f"<details><summary>Show Site_IDs</summary><div style='white-space: normal;'>{site_ids_str}</div></details>"
            group_ul += (
...
if not prolonged_issues_html:
    prolonged_issues_html = "<ul><li>No specific prolonged operational issues identified from current data (excluding 'Active' and 'PIC_Finder is not Active').</li></ul>"
...
for grade, color in [('A', '#d32f2f'), ('B', '#fbc02d'), ('C', '#388e3c')]:
    ...
    for site_id in top_sites:
        ...
        # Most frequent issue
        issues = df_filtered[df_filtered['Site_ID'].astype(str) == str(site_id)]['Issue_Identity']
        issues = issues[~issues.isin(['Active', 'PIC_Finder is not Active'])]
        ...
        ax_w.set_title(f"Weekly CA Trend: Site_ID {site_id}", fontsize=10)
        ...
        <div id="{popup_id}" style="display:none; position:fixed; top:0; left:0; width:100vw; height:100vh; background:rgba(0,0,0,0.25); z-index:9999;">
            <div style="background:#fff; margin:40px auto; padding:25px; border-radius:10px; max-width:520px; min-width:220px; position:relative; box-shadow: 0 5px 15px rgba(0,0,0,0.3);">
                ...
                <div style="font-size:1.1em; color:#222; text-align:center; margin:10px 0;"><b>Site_ID: {site_id}</b></div>
                ...
                <div style="margin-top:10px; font-size:0.95em; color:#444;">
                    <b>Interpretation:</b> This site shows CA fluctuation (Grade {grade}). Frequent issue: <b>{top_issue}</b>. Investigate for recurring Root_Causes or unstable conditions.
                </div>
            </div>
        </div>
        """

        # List item with clickable Site_ID
...
<p style="font-size:0.95rem;">
    ...
    <ul>
        ...
        <li>Click on a Site_ID to view its CA trend and most frequent operational issue. The chart is zoomable (use Ctrl+Scroll or pinch on touch devices).</li>
...
<p style="font-size:0.95rem;">
    ...
    Focus on Grade A sites for Root_Cause analysis and stabilization. Persistent CA volatility signals underlying systemic or environmental problems. Monitor Grades B and C for emerging risks.
</p>
"""
...
<div id="detail_data_analysis_tab" class="tabcontent">
    ...
    <p style="font-size:0.9rem;">A concentrated effort on the most frequent 'Issue Identities' (excluding 'Active' and 'PIC_Finder is not Active' which denote online status rather than actual issues) will yield the highest impact on overall maintenance stability:</p>
    {top_issues_html}
    <p style="font-size:0.9rem;"><em>Interpretation:</em> The dominance of these top issues indicates clear and recurring problem areas. Addressing these Root_Causes systematically, perhaps through enhanced training, equipment upgrades, or revised operational protocols, can significantly reduce overall downtime and improve service reliability.</p>

    <hr>

    <h3 style="color:#f57c00; font-size:1.1rem;">3. Render Type Performance and Issue Correlation</h3>
    <p style="font-size:0.9rem;">Analyzing issues by 'Render' type reveals whether certain operational models or equipment configurations are more susceptible to particular problems. This excludes 'Active' and 'PIC_Finder is not Active' statuses.</p>
    ...
    <p style="font-size:0.9rem;">Identifying issues that persist for extended periods is crucial, as they often indicate potential systemic problems, complex failures, or resource bottlenecks. This analysis specifically looks at non-active issues with a recorded 'Raise_Time':</p>
    {prolonged_issues_html}
    {scatter_plot_html}
    {mttr_mtbf_html}
    ...
    <blockquote style="border-left: 4px solid #1976d2; padding-left: 12px; color: #333; font-style: italic; font-size:0.9rem; background-color: #eef7ff; padding: 15px; border-radius: 8px;">
        ...
        <ol style="font-size:0.9rem; margin-top: 8px; padding-left: 20px;">
            <li><strong>Targeted Remediation & Root_Cause Analysis:</strong> Prioritize intensive efforts on the identified 'Top 5 Most Prevalent Issues' (e.g., Fiber Back Bone, Unsafe, Uplink Unsafe). Conduct thorough Root_Cause analyses for these recurring issues, especially in the 'Geographical Hotspots', to implement sustainable solutions rather than reactive fixes.</li>
...
<div id="site_status_summary" class="tabcontent">
    ...
    <p>This table shows Total Sites, Current Down Sites, and Online Status grouped by State/Division, Sub_Office, Township, and Render.</p>
...
<div id="analyst_overview" class="tabcontent">
    ...
    <h3 style="font-size:1.1rem;">2. Average CA by Sub_Office and Month</h3>
    ...
    <h3 style="font-size:1.1rem;">4. Average CA by Sub_Office and Week</h3>
                ...
                initComplete: function () {{
                    ...
                    api.columns().every(function () {{
                        ...
                        const filterableColumnNames = [
                            'Render', 'State/Division', 'Sub_Office', 'Township', 'Issue_Identity'
                        ];
...
output_dir = r'D:\My Base\Share_Analyst\SM Daily Report'
os.makedirs(output_dir, exist_ok=True) # Ensure directory exists

output_path = os.path.join(output_dir, f"site_perf_overview-{today}.html")
...
# Export monthly average CA by Sub_Office (pivot_suboffice_month) to Excel as a table
export_dir = r'D:\My Base\Share_Analyst\BD'
os.makedirs(export_dir, exist_ok=True)
export_path = os.path.join(export_dir, f"pivot_avg_ca_monthly_{today}.xlsx")
# Use the monthly pivot table (pivot_suboffice_month) for export
pivot_suboffice_month.to_excel(export_path, index=True)
print(f"✅ Exported monthly pivot_suboffice_month to Excel: {export_path}")